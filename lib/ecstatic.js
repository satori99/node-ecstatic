...
var ecstatic = module.exports = function (dir, options) {
  ...
  return function middleware (req, res, next) {
    ...
    */
    ...
    // Look for a gzipped or brotli file if this is turned on
    if ((opts.brotli || opts.gzip) && shouldCompress(req)) {

      function tryServeBrotli () {
        fs.stat(brotlied, function (err, stat) {
          if (!err && stat.isFile()) {
            // brotli has no magic bytes to check for
            file = brotlied;
            return serve(stat);
          } else if (opts.gzip) {
            tryServeGzipped();
    ...
    function serve(stat) {
	  ...
	  else if (file === brotlied) { // is .br picked up
...
// Check to see if we should try to compress a file with gzip or brotli.
function shouldCompress(req) {
  ...
  return headers && headers['accept-encoding'] &&
    headers['accept-encoding']
      .split(",")
      .some(function (el) {
        return ['*','compress', 'gzip', 'deflate', 'br'].indexOf(el) != -1;
      })
    ;
}
